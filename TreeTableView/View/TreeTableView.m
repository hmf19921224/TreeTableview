...
-(instancetype)initWithFrame:(CGRect)frame withData : (NSArray *)data{
    ...
    if (self) {
        ...
        [self registerNib:[UINib nibWithNibName:@"TreeTableViewCell" bundle:nil] forCellReuseIdentifier:@"node_cell_id"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TreeTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:NODE_CELL_ID forIndexPath:indexPath];
...
-(void)checkBtnClick:(UIButton*)sender{
    ...
    Node *node = [_tempData objectAtIndex:sender.tag];

    if (sender.isSelected) {
        
        if (_treeTableCellDelegate && [_treeTableCellDelegate respondsToSelector:@selector(AddNode:)]) {
            
            node.BtnState = sender.isSelected;
            [_treeTableCellDelegate AddNode:node];
        }
            
        }else
        {
   
        if (_treeTableCellDelegate && [_treeTableCellDelegate respondsToSelector:@selector(RemoveNode:)]) {
            
            node.BtnState = sender.isSelected;

            [_treeTableCellDelegate RemoveNode:node];
